...
test('Deep Equel', async t => {
    await t.expect(Selector('.header-wrapper').count).eql(5, 'Accordion Items Count', {timeout: 5000});
});
...
test.skip('OK', async t => {
    await t.expect(Selector('.header-wrapper').count).notEql(5, 'Accordion Items Count', {timeout: 5000});
});
