...
test('Get Devices List', async t => {
    ...
    response.body.forEach( async device => {
        ...
        console.log(device.system_name);
        const device_type = Selector(`div[class='list-devices'] div:nth-child(${device}) div:nth-child(1) span:nth-child(1)`);
        await t.expect(device_name.innerText).eql(device.system_type);
