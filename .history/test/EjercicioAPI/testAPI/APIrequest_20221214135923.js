...
response.body.forEach(device => {
    ...
    // await t.expect(Selector('span.device-name').innerText, response.body.system_name);
    // await t.expect(Selector('span.device-type').innerText, response.body.type);
    // await t.expect(Selector('span.device-capacity').innerText, response.body.dhh_caácity);
