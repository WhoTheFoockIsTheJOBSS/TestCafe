...
test.page('http://localhost:3001/')('Button in UI', async t => {
    ...
    response.body.forEach( async device => {        
        ...
        const editBtn = await Selector(`a[href="/devices/edit/${IDdevice}"]`).innerText;

        console.log(`${IDdevice} + ${editBtn}`);
        //t.expect(editBtn.innerText).eql('EDIT', 'Existe en el DOM');
        await t.expect(Selector(`a[href="/devices/edit/${IDdevice}"]`).innerText).eql('xxx', 'Accordion Items Count', {timeout: 5000});
