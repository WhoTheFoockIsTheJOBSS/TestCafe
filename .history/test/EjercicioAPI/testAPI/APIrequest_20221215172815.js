const { t, Selector, Xpath } = require("testcafe");
...
test.page('http://localhost:3001/')('Verifi list in UI', async t => {
    ...
    body.forEach( async device => {                 
        ...
        const device_name = Xpath(`//span[normalize-space()=${device.system_name}]`)
...
test.skip.page('http://localhost:3001/')('Button in UI', async t => {
    
    homePage.deviceTypeDD();

    const editBtn = Selector('a.device-edit')

    let cont = body.count
    await t.expect(Selector(editBtn).count).eql(cont);
});