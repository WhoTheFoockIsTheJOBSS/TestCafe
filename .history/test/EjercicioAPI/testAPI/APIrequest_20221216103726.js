...
test.page('http://localhost:3001/')('Button in UI', async t => {
    ...
    response.body.forEach( async device => {        
        ...
        await const editBtnExist = Selector(`a[href="/devices/edit/${IDdevice}"]`).exist;
        console.log(`${IDdevice} + ${editBtnExist}`);
        await t.expect(editBtnExist).ok();
        
        //await t.expect(editBtn).eql(ID)
    }); 
